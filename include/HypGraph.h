...
public:
  ...
  HypGraph(HypData *h, vector <string>);
  ...
  //vector <string> newickToLVHIST(vector<char> input,int currentDepth);
  bool isNhxCustom (std::vector <char> &input);
  vector <char> expandNhx(std::vector <char> &input);
  vector <char> expandNewick(std::vector <char> &input);
  vector <float> newickGetBounds(std::vector<char> &input);
  vector <string> newickToLVHIST(std::vector<char> &input);
  vector <string> nhxToLVHIST(std::vector<char> &input);
  ...
  int  addNode(string parentid, string childid, std::vector <std::string> XA);
  ...
  int  resetColorLink(HypLink *link);
  ...
  void setStaticColorGroup(int groupkey, int group, float r, float g, float b);//sjk2011
  void addColorGroup(int groupkey, string group, float r, float g, float b);

  int setDisableGroup(int groupkey, string group, int on);

  float *getColorGroup(int groupkey, string group);
  float *getStaticColorGroup(int groupkey, string group);//sjk2011
  
  float *getColorNode(HypNode *n);
  void setLinkSize(HypNode *n); //sjk2011
  ...
  HypNode *getNextNodeFromId(string id, int min)
  {
    int i=findNextUrl(id,min); return((i>=0)? nodes[i] : (HypNode*)NULL);
  }
  ...
  void newGraph(vector <string> input);
  void resetAllLinkColors(){
	  for(size_t i=0;i<links.size();i++){
		  resetColorLink(links.at(i));
	  }
  }
...
void loadSpanGraph(vector <string> fileLines);
void loadSpanGraph(vector <char> input);
vector <string> leafFilterLVHIST(vector <string> input);//sjk2011
...
HypNode *doAddNode(HypNode *current, string id, int level, int pri, std::vector<std::string> XA);
