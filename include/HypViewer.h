...
public:
  ...
  {
    thecenter = n;
	//selectedNode=n->getIndex();//sjk2011-debug
    return;
  }
  ...
  int flashPathLink(HypNode *fromnode, HypNode *tonode);
  int flashPath(HypNode *leftnode, HypNode *rightnode);
  int clearPath();
  void resetPath();
  ...
  int getHiliteNode();
  int getPick(){
	  //printf ("DEBUG::SELECTEDNODE :%d:\n",selectedNode);
	  return selectedNode;
  }
  void toggleDrawShape();  
  void setDrawShape(int input){if ((input >=0) && (input <5)) DRAW_SHAPE=input;}  
  void setLineWidth(float input);
  void drawSphereForExport(){drawSphere();}
  void setPDF(bool x){isPDF=x;}
  void clearFlashLink(){flashingLink=NULL;}
  void clearHiliteNode(){hiliteNode=-1;}
  void nodeScale();
private:
  bool isPDF;
  float lineWidth;
  ...
  void drawLabel(HypLink *l);
  void drawLabel(HypPoint p,string input,int);
  void drawStatus(int,string input,int);
  void drawStatus();
  ...
  void spinFunc(int x, int y, int shift, int control);
  ...
  static void spinFuncCB(int x, int y, int shift, int control);
  ...
  void frameBegin(HypData *);
  void frameContinue(HypData *);
  HypLink *flashingLink;

  vector <HypLink *> flashingPathLinks;
...
//  void drawStringInit();
  ...
  void sphereInit();
  void sphereInit(int list,float alpha);
  void framePrepare();
  void frameEnd();
  void idleCont();
  void redraw();
