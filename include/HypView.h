...
public:
	void toggleLeafColorScheme(){hypData->leafColorScheme=!hypData->leafColorScheme;}
//	bool leafColorScheme;
  ...
  int    gotoNextNode(const string & id,bool, int animate);//sjk2011

  int    gotoNextLandmark(int animate);//sjk2011
  int    gotoPrevLandmark(int animate);//sjk2011
  void   landmarkHighDnds();
  bool   landmarkPath();
  ...
  void   gotoSelectNode(int animate);
  ...
  int    setGraph(vector <string>);
  int    setGraph(vector <char>);
  //int    setGraphLeafColored(vector <string>);
  ...
  int  addNode(const string & parent, const string & child, std::vector <std::string> XA);
  ...
  //  
  void setRybGradient(int);//sjk-2011  deprecating
  void cycleNodeScheme();//sjk-2012

  void toggleChordateDenseView();//sjk-2011
  ...
  void addColorGroup(int i, const string & group, float r, float g, float b);
  ...
  void setColorBackgroundPlusFog(float r, float g, float b);
  ...
  void resetAllLinkColors(){hypGraph->resetAllLinkColors();}
  //  SJK2011  
  //  Returns the current newick graph
  //  usefull for saving a graph and for creating a subgraph
  //  If an LVHIST file was loaded this will return a blank.
  vector <char> getSpanGraph(){
	  return (hypGraph->spanGraph);
  }
  vector <char> getDrawgramSafeSpanGraph(){
	  vector <char> temp=hypGraph->spanGraph;
	  vector <char> result;
	  bool valid=true;
	  for(size_t i=0;i<temp.size();i++){
		  if ('['==temp[i]){ valid=false;}
		  if (']'==temp[i]){ valid=true; continue;}
		  if (valid) result.push_back(temp[i]);
	  }
	  return result;
	  
  }
  vector <char> startGraph;
  vector <char> getStartGraph(){
	  return (startGraph);
  }
  void toggleDrawShape(){
	  hypViewer->toggleDrawShape();
  }
  void setDrawShape(int input){
	  hypViewer->setDrawShape(input);
  }
  int selectedNode;

  //sjk2011  this is to fatten up the lines.
  void setLineWidth(float input){hypViewer->setLineWidth(input);}
  void clearPath(){hypViewer->clearPath();hypData->pathVisible=false;}
  void resetPath(){hypViewer->resetPath();hypData->pathVisible=false;}
  void showPath(){hypData->pathVisible=true;hypViewer->flashPath(leftPathNode,rightPathNode);}
  void showPath(string left, string right);
  void setLeftPathNode(HypNode * input){leftPathNode=input;}
  //void setLeftPathNode(char * input){leftPathNode=HypViewer->getNodeFromId(input);}
  void setRightPathNode(HypNode * input){rightPathNode=input;}
  void toggleTitleFlag(std::string input){
	  hypGraph->getNodeFromId(input)->setTitleFlag(!hypGraph->getNodeFromId(input)->getTitleFlag());
  }
    void clearAllTitleFlags(){
	    hypData->landMarkPath=false;
	    size_t size=hypGraph->NodeArray().size();
	    for (size_t i=0;i<size;i++){
		    hypGraph->NodeArray()[i]->setTitleFlag(false);
	    }
	   
    }
  //void tagLeftPathNode(){leftPathNode=lastFoundNode;}
  //void tagRightPathNode(){rightPathNode=lastFoundNode;}
//  void tagLeftPathNode(){leftPathNode=hypViewer->getCurrentCenter();}
  //void tagRightPathNode(){rightPathNode=hypViewer->getCurrentCenter();}
//  void tagLeftPathNode(){leftPathNode=hv->getPick();}
//  void tagRightPathNode(){rightPathNode=hv->getPick();}
  
    void cycleFogLevel(){hypData->fogLevel=(hypData->fogLevel+1)%4;}
    int getFogLevel(){return hypData->fogLevel;}
  void drawSphere(); 
  void drawStringInit(){hypViewer->drawStringInit();}
//int getBroccoliTops(){
// return hypData->broccoliTops;
//}
  void toggleBroccoliTops(){ // this is to make leaf nodes closer to the tree
	  if (hypData->broccoliTops>.8){hypData->broccoliTops=.6;}
	  else {hypData->broccoliTops=1.0;}
	  
	  
  }
  void isChordateTree() {
	  hypData->isChordateTree=true;
	  size_t size=hypGraph->NodeArray().size();
	  std::string test;
	  for (size_t i=0;i<size;i++){
		  test=hypGraph->NodeArray().at(i)->get_XA().at(3);
		  //if (test.find("dN/dS 0.000")==test.npos){hypData->isChordateTree=false;break;}
		  if (test.compare("dN/dS 0.000")){hypData->isChordateTree=false;break;}
	  }

	
}
  void  nodeScale(){hypViewer->nodeScale();}
  HypData    *hypData;//X1-Debug  normally private
private:
  ...
  int startNextIndex; //sjk2011  A value for the start Node
  //int startNextLandmarkIndex; //sjk2012  A value for the start Node

  HypNode * leftPathNode;
  HypNode * rightPathNode;
  HypNode * lastFoundNode;
  HypNode * lastFoundLandmark;
